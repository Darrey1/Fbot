...
class Bot(Client):   #inherit the base class Client from the fbchat
    def __init__(self, email, password, thread_id,thread_type,cookies):
    ...
    def onMessage(self, author_id, message_object, thread_id, thread_type,**kwargs):
         if author_id != self.uid:
             if (message_object.text.lower().find("still available") >=0):
             ...
             if message_object.text.lower().find('how much') >= 0:
                    sleep(random.randrange(5,15))
                    self.markAsDelivered(thread_id, message_object.uid)
                    self.markAsRead(thread_id)
                    self.setTypingStatus(TypingStatus.TYPING, thread_id=thread_id, thread_type=thread_type)
             ...
             if (message_object.text.lower().find('condition') >= 0) or (message_object.text.find('state') >= 0):
                    sleep(random.randrange(5,15))
                    self.markAsDelivered(thread_id, message_object.uid)
                    self.markAsRead(thread_id)
                    self.setTypingStatus(TypingStatus.TYPING, thread_id=thread_id, thread_type=thread_type)
             ...
             if (message_object.text.lower().find('address') >= 0):
                    sleep(random.randrange(5,15))
                    self.markAsDelivered(thread_id, message_object.uid)
                    self.markAsRead(thread_id)
                    self.setTypingStatus(TypingStatus.TYPING, thread_id=thread_id, thread_type=thread_type)
